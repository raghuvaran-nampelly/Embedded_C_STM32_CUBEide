...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  if(HAL_GPIO_ReadPin(GPIOC,ENT_SW_Pin) == 0)
	  {
		  HAL_GPIO_WritePin(GPIOB,RED_LED_Pin,GPIO_PIN_RESET);

	  }

	  if(HAL_GPIO_ReadPin(GPIOC,UP_SW_Pin) == 0)
	  {
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : UP_SW_Pin DN_SW_Pin ENT_SW_Pin */
  GPIO_InitStruct.Pin = UP_SW_Pin|DN_SW_Pin|ENT_SW_Pin;
